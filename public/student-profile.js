(async () => {
  ...
  profileForm.addEventListener("submit",async(event)=>{event.preventDefault();try{const form=new FormData(profileForm);const data=await G.api("/api/student/profile",{method:"PATCH",body:JSON.stringify({fullName:form.get("fullName"),department:form.get("department"),semester:form.get("semester"),giteaUsername:form.get("giteaUsername") || ""})});show(profileMessage,data.message,"success");document.querySelectorAll("[data-student-name]").forEach(el=>el.textContent=data.user.fullName);}catch(error){show(profileMessage,error.message,"error");}});
