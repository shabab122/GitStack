(async () => {
  ...
  function render() {
    ...
    grid.innerHTML = filtered.length ? filtered.map((student) => `
      <article class="student-card">
        ...
        <div class="mini-stats"><div class="mini-stat"><strong>${student.xp}</strong><span>XP</span></div><div class="mini-stat"><strong>${student.completedMissions}</strong><span>Completed</span></div><div class="mini-stat"><strong>${student.attempts}</strong><span>Attempts</span></div></div>
  ...
  try {
    const data = await G.api("/api/instructor/students");
