(async () => {
  ...
  function render(){
    grid.innerHTML = teams.length ? teams.map((team) => {
      ...
      <div class="notice info" style="margin-top:13px">Gitea repository: <strong>${team.giteaRepository ? G.escapeHtml(team.giteaRepository) : "not provisioned yet"}</strong>${team.giteaRepository ? ` · <a href="instructor-gitea.html">Manage repository</a>` : ` · <a href="instructor-gitea.html">Provision in Gitea</a>`}</div>
